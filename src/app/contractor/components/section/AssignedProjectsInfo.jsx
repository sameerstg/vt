...
export default function AssignedProjectsInfo() {
  ...
  const fetchMilestones = async (projectId) => {
    try {
      const res = await fetch(`/api/contractor/milestones?projectId=${projectId}`);
      const data = await res.json();
      if (data.success) {
  ...
  const handleUpdateMilestone = async (milestoneId, action) => {
    try {
      const res = await fetch("/api/contractor/milestones", {
        method: "PUT",
            ...
            <div className="ps-widget bgc-white bdrs4 p30 mb30 overflow-hidden position-relative">
              {loading ? (
                ...
                <div className="text-center p50">
                  ...
                  <Link href="/contractor/browse-projects" className="ud-btn btn-thm mt20">
                    Browse Projects
