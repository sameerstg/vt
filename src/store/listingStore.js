...
const listingStore = create((set) => ({
  ...
  setServiceMode: (payload) =>
    set((state) => {
      if (payload?.length !== 0) {
        const isExist = state.getServiceMode.includes(payload);
        if (!isExist) {
          return { getServiceMode: [...state.getServiceMode, payload] };
        } else {
          const deleted = state.getServiceMode.filter((item) => item !== payload);
          return { getServiceMode: deleted };
        }
      } else {
