...
import { assignedTaskMilestones } from "@/data/dashboardWorker";
...
export default function PaymentDistributionInfo() {
  const defaultMilestoneId = String(assignedTaskMilestones[0]?.id || "");
  ...
  const [transactionLog, setTransactionLog] = useState([
    {
      id: "dist-1",
      date: "2026-03-02",
      milestone: "Audit and issue breakdown",
      distributed: "$180",
      workers: 1,
      status: "Distributed",
    },
  ...
  const selectedMilestone = useMemo(
    () =>
      assignedTaskMilestones.find((item) => String(item.id) === String(selectedMilestoneId)),
    [selectedMilestoneId]
  );
  ...
  const validate = () => {
    ...
    if (!parseAmount(totalEscrowReceived)) {
      errors.totalEscrowReceived = "Total Escrow Received is required.";
    }
    if (!selectedMilestoneId) {
      errors.selectedMilestoneId = "Please select a milestone.";
    }
    if (!parseAmount(milestoneAmount)) {
      errors.milestoneAmount = "Milestone Amount is required.";
    }

    const invalidAllocation = allocations.some(
      (item) => !item.workerId || !parseAmount(item.amount)
    );
