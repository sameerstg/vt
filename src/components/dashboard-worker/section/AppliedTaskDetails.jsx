...
export default function AppliedTaskDetails() {
  ...
  const task = useMemo(() => {
    ...
    const staticTask = workerTasks.find(t => String(t.id) === String(taskId));
    if (staticTask) return staticTask;

    // 2. Check dynamic applied tasks for this worker
    const session = getAuthSession();
    if (session?.id) {
      const dynamicTasks = getWorkerAppliedTasks(session.id);
      const dynamicTask = dynamicTasks.find(t => String(t.id) === String(taskId));
